...
/**
 * router GET api/users/adminstAllUser
 * desc   管理员权限获取所有用户信息
 * access Private
 */
//1、 创建JWT验证的秘密路由
router.get('/ordinaryAllUser/:user_limit', passport.authenticate('jwt', { session: false }), (req, res) => {
    //获取所有普通用户信息
    users.find({ user_limit: req.params.user_limit }).then(user => {
        if (!user) {
            return res.json({
                code: 1,
                msg: '没有任何用户信息'
...
router.delete('/deleteUser', checkToken, (req, res) => {
    ...
    if (req.body.user_limit == 3) {
        users.remove({ _id: req.body._id }).then(result => {
            if (result) return res.json({ code: 0, msg: '已删除' })
